...
**Composition with worksheet 10.** Rows labeled `serious` should be cross-checked against `Incident type` from worksheet `10`. Mismatches (a `serious` harm but `Incident type = Delay`) often mean the model missed the actual incident type — read those rows.
...
**Pairing with worksheet 10.** Run `10-transform-incident-extraction.md` first on the factual-incident subset, then run this prompt on the same subset. The two columns cross-checked give a much richer picture of safety-relevant accounts than either alone — and the cross-check itself catches model errors in both columns.
